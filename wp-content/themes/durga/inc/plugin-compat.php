...
add_action( 'after_setup_theme', 'durga_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function durga_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'durga_construct_sidebars' );
}

if ( ! function_exists( 'durga_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'durga_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function durga_woocommerce_start() { ?>
...
if ( ! function_exists( 'durga_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'durga_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function durga_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'durga_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'durga_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function durga_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'durga_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'durga_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'durga_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function durga_bbpress_css() {
...
if ( ! function_exists( 'durga_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'durga_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function durga_buddypress_css() {
...
if ( ! function_exists( 'durga_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'durga_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function durga_beaver_builder_css() {
