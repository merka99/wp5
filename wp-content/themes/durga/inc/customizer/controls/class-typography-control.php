...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Durga_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Durga_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'durga-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'durga-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), DURGA_VERSION, true );
			wp_enqueue_style( 'durga-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), DURGA_VERSION );

			wp_enqueue_script( 'durga-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'durga-typography-selectWoo' ), DURGA_VERSION, true );
			wp_enqueue_style( 'durga-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), DURGA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'durga_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'durga' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'durga' );
			$this->json[ 'google_fonts' ] = apply_filters( 'durga_typography_customize_list', durga_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = durga_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'durga' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'durga' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'durga' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'durga' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="durga-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="durga-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="durga-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="durga-font-transform">
					<label>
