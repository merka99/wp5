...
if ( ! function_exists( 'durga_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function durga_get_setting( $setting ) {
		$durga_settings = wp_parse_args(
			get_option( 'durga_settings', array() ),
			durga_get_defaults()
		);

		return $durga_settings[ $setting ];
	}
}

if ( ! function_exists( 'durga_get_layout' ) ) {
	/**
	...
	function durga_get_layout() {
		...
		$layout = $durga_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_durga-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $durga_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $durga_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'durga_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'durga_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function durga_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $durga_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_durga-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'durga_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'durga_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function durga_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'durga_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $durga_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'durga_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'durga_show_title' ) ) {
	/**
	 ...
	 */
	function durga_show_title() {
		return apply_filters( 'durga_show_title', true );
	}
}

if ( ! function_exists( 'durga_padding_css' ) ) {
	/**
	 ...
	 */
	function durga_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'durga_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'durga_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function durga_get_navigation_location() {
		return apply_filters( 'durga_navigation_location', durga_get_setting( 'nav_position_setting' ) );
	}
}
