...
if(class_exists('Ultra_Companion')){
    ...
    add_action('woocommerce_single_product_summary','ultra_seven_social_share',50);
}
    ...
    if( $product->is_on_sale() && !$product->is_type('variable') && !$product->is_type('grouped') ):
        ...
        if($sale_label == 'discount'){
          ...
          echo  sprintf( __('<span class="onsale discount-wrap">- %s</span>', 'opstore' ), $saving_percentage );
            ...
            echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale">' . __( 'Sale', 'opstore' ) . '</span>', $post, $product );
        }
    endif;
}
...
function opstore_stock_info() {
    ...
    if( $product->is_in_stock() ):
        ...
        if( $product->get_manage_stock() ):
            ...
            $rem = (int) ( $stock - $sold );
            $percentage = number_format( ($rem / $stock * 100), 2 );
            ...
            ?>
            <div class="stock-info-bar mb-10">
                <div class="stock-counter">
                    <small class="pull-left <?php echo esc_attr( $stock_class ); ?>">
                        <?php /* translators: %d: product number */ ?>
                       <?php echo sprintf( esc_html__( 'Only %d left in stock', 'opstore' ), absint($rem) ); ?>
                    </small>

                    <div class="clearfix"></div>
                </div>
                <div class="opstore-stock-bar" style="width: 100%; border-radius: 8px; height: 8px; border: 1px solid #ccc;">
                    <div class="opstore-stock-percentage <?php echo esc_attr( $stock_class ); ?>" style="width: <?php echo esc_attr( $percentage ); ?>%; height: 100%; border-radius: 8px 0 0 8px; height: 6px;"></div>
                </div>
            </div>
            <?php
        else:
